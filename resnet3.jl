...
function bnorm(w, x, s, rng; mode=:train, momentum=.9, eps=1e-5)
    ...
    assert(length(rng) == 2)
    if mode == :test
        x_hat = (x .- s[rng[1]]) ./ (s[rng[2]] .+ eps)
        return w[1] .* x_hat .+ w[2]
    end
    m = size(x, 1) * size(x, 2) * size(x, 4)
    mu = sum(x, (1, 2, 4)) ./ m
    sigma2 = sum(x .* x, (1, 2, 4)) ./ (m + 1)
...
dtrn, dtrn_, dval, dtst = loaddata(;augment=false)
w, s, wranges, sranges, scnts = init_model(;dtype=dtype)
lossgrad = grad(loss)

# Global state service for the use of layers
global_state = Dict{Any, Any}()
w = train(w, dtrn_, dval; actual_trn=dtrn, dtype=dtype, bsize=64, iters=20000, print_period=250, augmented=false)
#global_state[:nforward] = 0
