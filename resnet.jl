module ResNet
   ...
   function init_model(;n=3, dtype=Array{Float32}, pre=false, add_block! = add_basic_block!)
      w = Any[]
      s = Any[]
      ranges = Any[]
      sranges = Any[]
   ...
   function train(w, dtrn, dtst; s=nothing, dtype=Array{Float32}, iters=15000, bsize=32, print_period=1000, lr=0.1,
      ...
      report = nothing
      if val_only
         report = iter->println((:iter,iter,:trn,:val,accuracy(w,dtst; dtype=dtype, bmode=:test)))
      else
   ...
   global bsize = 128
   # Model specs
   function init(_n=3, _dtype=nothing, _lambda=.0001, _nval=5000, _bsize=128)
      global n = _n
